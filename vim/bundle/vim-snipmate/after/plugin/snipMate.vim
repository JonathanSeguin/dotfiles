...
if !exists('g:snips_no_mappings') || !g:snips_no_mappings
	if exists('g:snips_trigger_key')
		...
		" + (jo) change tab to shift-tab for snippets
		let s:overwrite = maparg('<Tab>', 'i') ==? '<Plug>SuperTabForward'
		call s:map_if_not_mapped('<S-Tab>', '<Plug>snipMateNextOrTrigger', 'i')
		call s:map_if_not_mapped('<S-Tab>', '<Plug>snipMateNextOrTrigger', 's')
		let s:overwrite = 0
		call s:map_if_not_mapped('<S-Tab>', '<Plug>snipMateVisual', 'x')
	endif
